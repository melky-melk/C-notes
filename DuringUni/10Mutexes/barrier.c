...
void* red_thread(void *args){
	...
	// meaning because it starts at 2, it will decriment once
	// and wait till another dancer has taken stage
	sem_wait(&red_sem);

	//then wait til 3 total dancers on the stage (the barrier is like a semaphore that waits til its 0)
	...
	//once there is a white then you can unlock all 3, 	red();
	return NULL;
}
